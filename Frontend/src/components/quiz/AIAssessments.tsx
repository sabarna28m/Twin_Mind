          ...
          {q.options!.map((opt, oi) => {
            ...
            return (
              <button key={oi} onClick={() => onAnswer(letter)} style={{
                ...
                borderColor: selected ? 'rgba(0,212,255,0.5)' : 'rgba(255,255,255,0.1)',
        ...
        <div style={{ display: 'flex', gap: '0.6rem' }}>
          {['True', 'False'].map(v => (
            <button key={v} onClick={() => onAnswer(v)} style={{
              ...qc.tfBtn,
              background: answer === v ? (v === 'True' ? 'rgba(52,211,153,0.15)' : 'rgba(248,113,113,0.15)') : 'rgba(255,255,255,0.03)',
              borderColor: answer === v ? (v === 'True' ? 'rgba(52,211,153,0.5)' : 'rgba(248,113,113,0.5)') : 'rgba(255,255,255,0.1)',
...
const qc: Record<string, React.CSSProperties> = {
  wrap:      { display: 'flex', flexDirection: 'column', gap: '0.85rem', padding: '1.25rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px' },
  head:      { display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' as const },
  qNum:      { fontSize: '0.7rem', fontWeight: 800, color: 'rgba(148,163,184,0.5)' },
  ...
  textInput: { padding: '0.6rem 0.85rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', color: 'var(--text-h)', fontSize: '0.85rem', fontFamily: 'inherit', outline: 'none' },
  textarea:  { padding: '0.65rem 0.85rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', color: 'var(--text-h)', fontSize: '0.84rem', fontFamily: 'inherit', outline: 'none', resize: 'vertical' as const, lineHeight: 1.5 },
};
...
function ReviewCard({ q, userAnswer }: { q: QuizQuestion; userAnswer: string | undefined }) {
  ...
  return (
    <div style={{
      ...qc.wrap,
      borderColor: !isObjective ? 'rgba(255,255,255,0.08)' : isCorrect ? 'rgba(52,211,153,0.3)' : 'rgba(248,113,113,0.3)',
      background:  !isObjective ? 'rgba(255,255,255,0.02)' : isCorrect ? 'rgba(52,211,153,0.05)' : 'rgba(248,113,113,0.05)',
    }}>
      <div style={qc.head}>
      ...
      {/* User's answer */}
      <div style={{ padding: '0.5rem 0.75rem', background: 'rgba(255,255,255,0.04)', borderRadius: '8px' }}>
        <p style={{ margin: '0 0 0.15rem', fontSize: '0.62rem', fontWeight: 700, color: 'rgba(148,163,184,0.5)', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Your Answer</p>
      ...
      <button onClick={() => setShowExp(v => !v)} style={{
        padding: '0.3rem 0.6rem', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)',
        borderRadius: '7px', fontSize: '0.7rem', color: 'rgba(148,163,184,0.6)', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' as const,
      }}>
        {showExp ? '▲ Hide' : '▼ Show'} Explanation
      </button>
      {showExp && (
        <p style={{ margin: 0, fontSize: '0.8rem', color: 'rgba(226,232,240,0.65)', lineHeight: 1.55, padding: '0.5rem', background: 'rgba(255,255,255,0.03)', borderRadius: '8px' }}>
          {q.explanation}
        </p>
      )}
    </div>
          ...
          <div style={s.matGrid}>
            {materials.map(m => (
              <div
                ...
                style={{
                  ...s.matCard,
                  borderColor: selected?.id === m.id ? 'rgba(0,212,255,0.5)' : 'rgba(255,255,255,0.08)',
                  background:  selected?.id === m.id ? 'rgba(0,212,255,0.08)' : 'rgba(255,255,255,0.03)',
                }}
              >
      ...
      <div style={s.card}>
        ...
        <div style={s.configGrid}>
            ...
            <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' as const }}>
              {(['easy', 'medium', 'hard', 'mixed'] as Difficulty[]).map(d => (
                <button key={d} onClick={() => setDifficulty(d)} style={{
                  ...s.diffBtn,
                  borderColor: difficulty === d ? DIFF_COLORS[d] : 'rgba(255,255,255,0.1)',
          ...
          <div style={s.configField}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.62rem', color: 'rgba(148,163,184,0.35)', marginTop: '0.1rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '0.35rem' }}>
              {Q_TYPE_OPTIONS.map(t => (
                <button key={t} onClick={() => toggleType(t)} style={{
                  ...s.typeBtn,
                  borderColor: types.includes(t) ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.1)',
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <span style={{ fontSize: '0.7rem', color: 'rgba(148,163,184,0.5)', fontWeight: 600, whiteSpace: 'nowrap' as const }}>
            ...
            {questions.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  background: i === current ? 'rgba(0,212,255,0.2)' : answers[questions[i].id] !== undefined ? 'rgba(52,211,153,0.12)' : 'rgba(255,255,255,0.04)',
                  borderColor: i === current ? 'rgba(0,212,255,0.5)' : answers[questions[i].id] !== undefined ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.1)',
                  color: i === current ? '#00D4FF' : answers[questions[i].id] !== undefined ? '#34d399' : 'rgba(148,163,184,0.5)',
                }}
              >{i + 1}</button>
            ))}
          </div>
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={{ ...s.card, textAlign: 'center', padding: '2rem 1.5rem' }}>
          ...
          <p style={{ margin: '0 0 1.25rem', fontSize: '0.8rem', color: 'rgba(148,163,184,0.5)' }}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  pageHead:   { paddingBottom: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.07)' },
  ...
  card:       { background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.85rem' },
  cardTitle:  { margin: 0, fontWeight: 800, fontSize: '0.9rem', color: 'rgba(226,232,240,0.88)' },
  hint:       { margin: 0, fontSize: '0.78rem', color: 'rgba(148,163,184,0.4)', textAlign: 'center' as const },
  ...
  configLabel:{ fontSize: '0.74rem', fontWeight: 700, color: 'rgba(148,163,184,0.6)', textTransform: 'uppercase' as const, letterSpacing: '0.06em' },
  textInput:  { padding: '0.55rem 0.85rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', color: 'var(--text-h)', fontSize: '0.84rem', fontFamily: 'inherit', outline: 'none' },
  ...
  outlineBtn: { padding: '0.55rem 1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '9px', color: 'rgba(148,163,184,0.7)', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' },
  ...
  quizMeta:   { margin: 0, fontSize: '0.7rem', color: 'rgba(148,163,184,0.45)' },
};
