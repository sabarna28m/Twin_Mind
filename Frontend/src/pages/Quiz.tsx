...
export default function Quiz() {
  ...
  const [answered, setAnswered]       = useState(false);   // has picked for current Q
  ...
  async function submitResults() {
    try {
      ...
      setHistory(r.data);
    } catch { /* saved to history on next load */ }
  }
    ...
    return (
      <div style={p.shell}>
        <header style={p.nav}>
          ...
          <span style={p.navTitle}>Quiz</span>
          <div style={{ width: 100 }} />
        </header>
        <div style={p.center}>
          <div style={p.genBox}>
  ...
  if (phase === 'quiz') {
    ...
    return (
      <div style={p.shell}>
        <header style={p.nav}>
          <div style={p.navLeft}><BackButton /></div>
          <span style={p.navTitle}>Quiz — {subject}</span>
            ...
            <div style={p.optionsGrid}>
              {q.options.map((opt, i) => {
                ...
                if (answered) {
                  ...
                  else if (i === chosenIdx && i !== q.correct) { bg = 'rgba(239,68,68,0.1)'; border = '#ef4444'; color = '#ef4444'; }
                } else if (i === chosenIdx) {
                ...
                return (
                  ...
                  >
                    ...
                    {answered && i === q.correct && <span style={{ marginLeft: 'auto', fontSize: '1rem' }}>✓</span>}
                    {answered && i === chosenIdx && i !== q.correct && <span style={{ marginLeft: 'auto', fontSize: '1rem' }}>✗</span>}
                  </button>
                );
              })}
            </div>
                ...
                return (
                  <div key={qi} style={{ ...p.reviewCard, borderColor: `${statusColor}44` }}>
                    <div style={p.reviewHeader}>
                      <span style={{ ...p.reviewNum, background: `${statusColor}22`, color: statusColor }}>
                        {statusIcon} Q{qi + 1}
                      </span>
                ...
                <div style={p.chipRow}>
                  {subjects.map(s => (
                    <button
                      ...
                      style={{
                        ...p.chip,
                        background: subject === s ? 'rgba(99,102,241,0.15)' : 'var(--bg-surface)',
                        border: `1.5px solid ${subject === s ? '#6366f1' : 'var(--border)'}`,
                        color: subject === s ? '#818cf8' : 'var(--text-h)',
                      }}
                    >
                      {s}
                    </button>
                ...
                {DURATIONS.map(d => (
                  <button
                    ...
                    style={{
                      ...
                      background: duration === d.value ? 'rgba(99,102,241,0.15)' : 'var(--bg-surface)',
                      border: `1.5px solid ${duration === d.value ? '#6366f1' : 'var(--border)'}`,
                      color: duration === d.value ? '#818cf8' : 'var(--text-h)',
                    }}
                  >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {DIFFICULTIES.map(d => (
                  <button
                    ...
                    style={{
                      ...p.diffBtn,
                      background: difficulty === d.value ? `${d.color}14` : 'var(--bg-surface)',
                ...
                {history.map(h => {
                  ...
                  return (
                    <div key={h.id} style={p.histItem}>
                      ...
                      <div style={p.histBar}>
                        <div style={{ ...p.histBarFill, width: `${h.percentage}%`, background: col }} />
                      </div>
                    </div>
                  );
                })}
